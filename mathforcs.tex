\DocumentMetadata{lang=en,tagging=on,pdfstandard=ua-2,tagging-setup={math/setup=mathml-SE,math/alt/use}}
\documentclass[10pt]{memoir}
\usepackage{amsmath,amsfonts,amssymb,amsthm}
%\usepackage[tight]{diagrams}
\usepackage{MnSymbol}
\usepackage[mathlf,textlf,minionint]{MinionPro}
\DeclareMathSymbol{\mycomplement}   {\mathord}{AMSa}{"7B}
%\usepackage[T1]{fontenc}
%\usepackage{textcomp}
\usepackage{calc,url}
\usepackage{algpseudocode}
\usepackage{algorithmicx}
\usepackage[pdftex]{color,graphicx}
\input{lecmath.sty}
\usepackage{lecmemoir,lecpagesize}
\theoremstyle{mybem}
\usepackage{exercise}
\usepackage{makeidx}
\usepackage{hyperref}
\hypersetup{colorlinks=true, unicode=true, linkcolor=[rgb]{0.10,0.05,0.67}, citecolor=[rgb]{0.10,0.05,0.67}, filecolor=[rgb]{0.10,0.05,0.67}, urlcolor=[rgb]{0.10,0.05,0.67}}
\usepackage{pdfcomment}
\pdfcompresslevel=9

% temporary workaround until accessibility support in LaTeX gets cleaned up
\newcommand\anngraphics[3]{%
%\pdftooltip{\includegraphics[width=#1]{#2}}{#3}%
\includegraphics[width=#1,alt={#3}]{#2}%
}

%

\let\cal=\mathcal
\def\qed{\hfill{$\square$}}

\def\mynote#1{\footnote{#1}}
\def\person#1{#1}
\def\figuref#1{Figure~\ref{#1}}
\def\pointer#1{{\color{blue}\underline{$\leadsto$~\ref{#1%
}}}}
\def\bonussection{\begin{center}\textbf{BONUSSECTION}\end{center}}

\makeindex
\title{Mathematics for Computational Science}
\author{Alexander Hulpke}
\date{\today}
\begin{document}
\chapterstyle{BlueBox}%bianchi}
\frontmatter
%\thispagestyle{empty}
%\vfill
%\begin{center}
%{\Huge
%! Do not Print yet !
%\bigskip\bigskip
%
%Preliminary Version
%\bigskip\bigskip
%
%Subject to final Proof-reading
%\bigskip\bigskip
%
%Final Version will be available
%
%in time for SP'22
%}
%\end{center}
%\vfill
%\newpage

\thispagestyle{empty}
%\maketitle
\ \\
\vspace{1mm}
\begin{center}
{\huge\textbf{\thetitle}}
\bigskip

{\large Lecture Notes for MATH 156}
\bigskip
\bigskip

{\Large\theauthor}\\
{with\\
K.~Bonney,
H.~Meade,
M.~Moy,
T.~Neighbors,
R.~Tremaine}
\medskip

{\large\thedate}
\end{center}

\ \hspace{8cm}
\vspace{2mm}
\begin{center}
%\includegraphics[width=10.5cm]{pic/Kandinsi_CirclesInACircle.jpg}
\anngraphics{10.5cm}{pic/Kandinsi_CirclesInACircle.jpg}{Decorative cover
image: Kandinsky, Circles in a circle, alluding to subset relations}
\end{center}
\vspace{5mm}
%\vfill

%\hfill {\tiny Version of \today}
\newpage
\thispagestyle{empty}
\begin{raggedright}
{\small
Alexander Hulpke\\
Department of Mathematics\\
Colorado State University\\
1874 Campus Delivery\\
Fort Collins, CO, 80523
}
\end{raggedright}
\vfill
\setlength\epigraphwidth{4cm}
\begin{epigraphs}
\qitem{%
\textit{Title graphics:}\\
}
{Circles in a Circle (1923)}
\textsc{Wassily Kandinsky}
\end{epigraphs}
\vspace{1cm}
\begin{raggedright}
{\small
\bigskip

\copyright 2022 Alexander Hulpke.\\
This work is licensed under a
Creative Commons Attribution-NonCommercial-ShareAlike License (CC~BY-NC-SA,
{\footnotesize \url{https://creativecommons.org/licenses/by-nc-sa/4.0/}})\\
\includegraphics[width=2cm]{pic/by-nc-sa.png}
}
\end{raggedright}
\newpage
%\setlength{\cftpartnumwidth}{1.5em}
\setlength{\cftchapternumwidth}{2em}
\setlength{\cftsectionnumwidth}{2.5em}
\setlength{\cftsubsectionindent}{4em}
\tableofcontents
\input{preface}
\endfrontmatter
\mainmatter
\input{chsets.tex}
\input{chrels.tex}
\input{chfuns.tex}
\input{chseqs.tex}
\input{chdiff.tex}
\input{chappdiff.tex}
\input{chtaylor.tex}
\input{chanti.tex}
\endmainmatter

\backmatter
\long\def\thechapter{I} % to force the bar width small
%\bibliographystyle{amsalpha}
%\bibliography{combook}
\printindex
\endbackmatter
\end{document}
