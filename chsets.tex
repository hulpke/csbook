...
hard or impossible. Thus two other techniques are used. The first of
...
$:$ are used as well). One can read this separator as ``such that''.
...
Finally, there is a variant that describes elements by transforming
...
element, and $\N_{>0}$ (or $\Z_{>0}$) if we explicitly want to
exclude $0$.
...
equality of sets to two subset tests:
\begin{lemma}
...
reason for building pairs from sets is that it allows us to re-use existing
...
one would generate such objects in algorithms. First, let's look how an
