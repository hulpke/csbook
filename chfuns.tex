...
\anngraphics{6cm}{pic/GeneralFct.png}{Arrows are drawn from points within a
cloud labelled Domain (Source) to a subset of points within a cloud labelled
Target. The subset is the Image, or Range.}
\end{center}
\caption{A function}
\label{figgeneralfct}
\end{figure}
...
domains, their targets\mynote{In the context of functions, the target is
sometimes called \defini{codomain}.}, and finally show that for \textbf{every} $x$ in the
...
\anngraphics{6cm}{pic/AnimalColor.png}{Arrows are drawn from "frog" in the
Domain to "green" in the Range, from "horse" in the Domain to "red" in the
Range, and from "sheep" in the Domain to "blue" in the Range.}
\end{center}
...
For $func(n)$ to truly describe a function, we must assume some unique naming
...
%onto. For example, we could require that $1^2=1$ be written as `one', $2^2 = 4$ be written
%as `four,' and so on for the first 26 square numbers. In this case, $func(n)$ would
...
A key difference between $\mbox{\texttt{func}}(n)$ and
...
\medskip
Let $A,B$ be sets and $b\in B$ some element. A \defini{constant function}
...
\anngraphics{3cm}{pic/fctshift0.png}{Original function graph}
\end{minipage}&\begin{minipage}[t]{3.5cm}
b) $f(x)+1$\\
%\includegraphics[width=3cm]{pic/fctshift1.png}
\anngraphics{3cm}{pic/fctshift1.png}{Graph shifted up one unit}
\end{minipage}&\begin{minipage}[t]{3.5cm}
c) $f(x)-3$\\
%\includegraphics[width=3cm]{pic/fctshift2.png}
\anngraphics{3cm}{pic/fctshift2.png}{Graph shifted down 3 units}
\end{minipage}\\
\\
\begin{minipage}[t]{3.5cm}
d) $-f(x)$\\
%\includegraphics[width=3cm]{pic/fctshift3.png}
\anngraphics{3cm}{pic/fctshift3.png}{Graph reflexted on x axis}
\end{minipage}&\begin{minipage}[t]{3.5cm}
e) $2-f(x)$\\
%\includegraphics[width=3cm]{pic/fctshift4.png}
\anngraphics{3cm}{pic/fctshift4.png}{Graph reflexted on x axis and shifted
up 2 units}
\end{minipage}&\begin{minipage}[t]{3.5cm}
f) $2f(x)$\\
%\includegraphics[width=3cm]{pic/fctshift10.png}
\anngraphics{3cm}{pic/fctshift10.png}{Graph stretched vertically by a factor
of 2}
\end{minipage}\\
\\
\begin{minipage}[t]{3.5cm}
...
\anngraphics{3cm}{pic/fctshift11.png}{Graph flipped vertiocally and
...
section \mynote{these are called \defini{linear} functions}, the effect of
...
\anngraphics{\the\textwidth}{pic/OntoPics.png}{Each of four diagrams
contains two ovals, labelled A and B respectively. In the diagram labelled
"onto", arrows are drawn from all 5 points in A to all 4 points in B. In the
diagram labelled "not onto", arrows are drawn from each of the 5 points in A
to 3 of the 4 points in B. In the diagram labelled "one-to-one", arrows are
drawn from each of the 4 points in A to four of five separate points in B.
In the diagram labelled "not one-to-one", arrows are drawn from each of the
4 points in A to 3 of the 5 points in B, resulting in two points from A
being mapped to the same point in B.}
...
\anngraphics{8cm}{pic/OneToOneOnto.png}{The concepts of "onto" and
"one-to-one" are illustrated through the graphs of four functions defined on
the real numbers.}
...
numbers, car number plates\mynote{That is the reason for a number plate such as
\texttt{COOLGUY14} -- the number 14 is not special, but 1--13 were used
already.},
...
\defini{bijective} or a \defini{bijection}. Being bijective means that for every $b\in
...
are\mynote{We have that $0!=1$ as product over an empty set, in the same way
