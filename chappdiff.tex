...
\anngraphics{8cm}{pic/kurvdiss1.png}{The graph of a function looks like a
...
\anngraphics{8cm}{pic/Kurdi2.pdf}{The graph of the derivative, $f'(x)$, is
added to the coordinate plane. Places wehere f increases are where the
derivative is positive, places where f decreases have negative derivative.}
\end{center}
...
\anngraphics{8cm}{pic/Kurvendiskussion.pdf}{The graph of the second
derivative, $f''(x)$, is added to the coordiante plane. Critical points of
$f(x)$ occur when the graph of $f'(x)$, the first derivative, touch the
x-axis. Turning points of $f(x)$ occur when the graph of $f''(x)$, the
second derivative, crosses the x-axis.}
\end{center}
...
\anngraphics{8cm}{pic/FuncSynth.pdf}{The figures constructed in the table are
matched to where they are seen on the graph in the example.}
\end{center}
...
\anngraphics{8cm}{pic/tnex3pic.png}{Critical Points and turning points are
drawn on the graph}
\end{center}
...
\anngraphics{8cm}{pic/newton4.png}{Four functions are plotted on a
coordinate plane whose graphs all intersect in the same two points. But each
function crosses the x-axis at a different x-value.}
\end{center}
...
\anngraphics{6cm}{pic/newtonmeth.pdf}{The Newton Method is illustrated on a
coordinate plane via the creation of a tangent line with a positive slope
which passes through $(x_{n+1}, 0)$ and $(x_n, f(x_n))$.}
\end{center}
...
that $\sz{f(x)}\le c \sz{g(x)}$ for all $x\ge N$. We write\mynote{The use
of the equal sign is somewhat misleading and awkward, but that is the
convention that is usually used, so we stick with it.}
$f(x)=\bigo{g(x)}$.
\end{defn}
...
$x$}
is a
function that, for a given algorithm indicates the cost (number of
...
modified kinds of comparisons.} We consider this more formally below.
\smallskip
...
\anngraphics{10cm}{pic/GrowthOrders.pdf}{Illustrating the varying growth of
functions}
\end{center}
...
We have thus shown that the relation $\sim$ satisfies all the requirements to
be an equivalence relation. 

The relation $\sim$ thus creates a partition of the set of function onto
equivalence classes (which we shall call \defini{complexity classes}). The
notation
...
%What if we relaxed the condition for $\sim$ to hold?
%Perhaps we 
...
%\smallskip
%
%Returning to the equivalence relation given by $\sim$, recall that
%this gives us a notion of equivalence classes. 

%We can partition the
%set of all functions into smaller sets of functions which
%are all related to each other by $\sim$. We will refer to these equivalence
%classes as complexity classes 
%%TODO: link to table of complexity classes
%\smallskip

%Thinking with complexity classes greatly simplifies our view of functions. This change in focus towards complexity classes
%emphasizes growth rather than the nuance of any particular function.
%Typically we distinguish $x$, $x+1$, and $50x$ as separate functions, however,
%they all belong to the same complexity class $\bigo(x)$. 
%We can think
%of these minor variations as corresponding to the varying circumstances under which
%a program may be executed, such as the programming languages or computer that is used.
